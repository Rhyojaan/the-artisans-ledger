...
function aldrenJournalNote(){
 let p=profile();if(!p)return "";
 ensureProfileShape();
 let known=countKnown(),total=Object.keys(R).length*4,pct=Math.round(known/total*100),c=p.character||{};
 if(pct>=100)return `${c.name||p.name} has completed the alchemical archive. Few records in Tamriel are this complete.`;
 if(pct>=75)return `${c.name||p.name} is nearing mastery. The remaining discoveries deserve careful attention.`;
 if(pct>=40)return `${c.name||p.name}'s archive is taking shape. Patience and steady experimentation will finish the work.`;
 if(known>0)return `${c.name||p.name} has begun the work. Every recorded trait strengthens the archive.`;
 return `${c.name||p.name} stands at the beginning of the path. The first discovery awaits.`;
}
...
function getAlchemyLaboratoryUseEnabled(){let p=profile();return !!(p&&p.settings&&p.settings.laboratoryUseUnlocked)}
function setAlchemyLaboratoryUseEnabled(enabled,refresh=true){let p=profile();if(!p)return;ensureProfileShape();p.settings.laboratoryUseUnlocked=!!enabled;localStorage.setItem("laboratoryUseUnlocked",p.settings.laboratoryUseUnlocked?"true":"false");if(refresh){save();refreshTraitCompletionPlan()}else{let box=document.getElementById("laboratoryUseUnlocked");if(box)box.checked=p.settings.laboratoryUseUnlocked}}
function saveLaboratoryUseUnlocked(){let box=document.getElementById("laboratoryUseUnlocked");setAlchemyLaboratoryUseEnabled(!!(box&&box.checked),true)}
function setupLaboratoryUseControl(){let box=document.getElementById("laboratoryUseUnlocked");if(!box)return;box.checked=getAlchemyLaboratoryUseEnabled();if(!box.dataset.aldrenLabHooked){box.addEventListener("change",e=>setAlchemyLaboratoryUseEnabled(e.target.checked,true),true);box.dataset.aldrenLabHooked="true"}}
function renderLaboratoryUseControl(){setupLaboratoryUseControl()}
function recipeAllowedByLaboratoryUse(c){return getAlchemyLaboratoryUseEnabled()?c.length<=3:c.length===2}
function refreshTraitCompletionPlan(){let summary=document.getElementById("planSummary"),list=document.getElementById("planList");if(summary&&list)buildPlan()}
...
function ranked(state=data(),invOnly=inventoryOnly){return ALL.filter(recipeAllowedByLaboratoryUse).map(c=>scoreCombo(c,state,invOnly)).filter(Boolean).sort((a,b)=>b.score-a.score)}
...
function gatherAdvice(){let theoretical=ranked(data(),false)[0];if(!theoretical)return "The archives may be complete, or no useful experiment remains.";let missing=theoretical.c.filter(r=>Number(data()[r].qty||0)<1);return `Aldren found no useful discovery you can craft with your current reagents.<br><br><span class=small>To pursue the strongest next experiment, gather:</span><br>${missing.map(x=>`<span class=pill>${x}</span>`).join("")}`;}
function buildPlan(){let summary=document.getElementById("planSummary"),list=document.getElementById("planList");if(!summary||!list)return;let state=cloneState(data()),steps=[],safety=0;while(safety++<80){let r=ranked(state,inventoryOnly)[0];if(!r)break;steps.push(r);applyCraftToState(state,r)}summary.innerHTML=steps.length?`Plan found: <b>${steps.length}</b> experiments. Estimated new traits: <b>${steps.reduce((a,s)=>a+s.disc.length,0)}</b>.`:"No useful plan found. Try turning inventory-only OFF.";list.innerHTML=steps.slice(0,30).map((s,i)=>`<div class=planStep><b>Experiment #${i+1}</b><br>${s.c.join(" + ")}<br><span class=small>${s.disc.length} discoveries: ${s.disc.map(d=>d[1]).join(", ")}</span></div>`).join("")}
