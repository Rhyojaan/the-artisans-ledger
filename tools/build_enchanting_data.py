...
JSON file requested by the caller, defaulting to data/enchanting-data.json when
...
4. Review the generated JSON diff before wiring it into script.js.
...
def write_json(data: Mapping[str, Any], output_path: Path) -> Path:
    ...
    output_path.write_text(json.dumps(data, indent=2, ensure_ascii=False, sort_keys=False) + "\n", encoding="utf-8")
    return output_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Enchanting data JSON for The Artisan's Ledger.")
    parser.add_argument(
        "--output",
